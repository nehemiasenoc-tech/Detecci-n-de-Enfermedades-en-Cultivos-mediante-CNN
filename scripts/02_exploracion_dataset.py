...
Análisis:
- Clases, cantidad de imágenes, balance, resolución, cultivos representados
"""
import os
import json
import numpy as np
...
def find_image_dirs(dataset_dir):
    """Busca directorios con imágenes en el dataset."""
    train_dir = None
    val_dir = None
    test_dir = None
        ...
        for root, dirs, files in os.walk(dataset_dir):
            ...
            if len(subdirs) > 0:
                for sd in subdirs:
                    ...
                    sub_subdirs = [d for d in os.listdir(sd_path) if os.path.isdir(os.path.join(sd_path, d))]
                    if len(sub_subdirs) > 5:
                        has_images = False
                        for ssd in sub_subdirs:
                            ssd_path = os.path.join(sd_path, ssd)
                            img_files = [f for f in os.listdir(ssd_path) if f.lower().endswith(('.jpg', '.jpeg', '.png'))]
                            if len(img_files) > 0:
                                has_images = True
                                break
                        if has_images:
                            if 'train' in sd.lower():
                                train_dir = sd_path
                            elif 'valid' in sd.lower() or 'val' in sd.lower():
                                val_dir = sd_path
                            elif 'test' in sd.lower():
                                test_dir = sd_path
...
def main():
    ...
    if train_dir is None:
        print("\nNo se encontraron directorios de entrenamiento.")
